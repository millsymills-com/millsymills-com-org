...
- **ADR-0001** — `docs/adr/0001-gate-bypass-mitigation.md`. `workflow_run` mitigation for PR-modifiable gate bypass (issue #13). Impl LIVE via `.github/workflows/gate-verified.yml`. Rollout step 2 LIVE since PR #52 merged 2026-05-15 — `gate-verified` is now a required status check on the management repo's default branch alongside `gate`; observation week ends ~2026-05-22. Step 3 (drop `gate`) is issue #35. Codex adversarial review on #52 surfaced that gate-verified only catches the skip/rename/missing-conclusion class; the stub-with-success residual is documented in ADR-0001 *What `gate-verified` catches and does not catch* and tracked separately in issue #53.
...
PR opens → `tofu-plan` runs (reader App, plan-only OIDC role pinned to the `tofu-plan` GitHub environment) → `gate` synthesizer job encodes "validate must succeed; plan must succeed on internal PRs or be skipped on fork PRs" and is the in-PR required check → `gate-verified.yml` re-asserts `gate` succeeded from `main`'s default-branch context via `workflow_run` and posts a `gate-verified` check (also required, additive with `gate` per ADR-0001 — closes the skip/rename/missing-conclusion bypass class, not stub-with-success; stub-with-success residual tracked in issue #53) → ruleset gates merge → `tofu-apply` runs on `main` (writer App, apply OIDC role pinned to `tofu-apply.yml@refs/heads/main`) → nightly `tofu-drift` with `-detailed-exitcode` opens an issue on non-zero.
...
- `gate-verified.yml` runs on `main` only, listens to `workflow_run` completions of `tofu`, and posts a `gate-verified` check-run after asserting the in-run `gate` job succeeded. A PR cannot edit a default-branch-only workflow, so the verifier itself is tamper-proof; what it can verify is bounded by the job-list view (name + conclusion). This closes the skip/rename/missing-conclusion class of PR-side `gate` mutation; the stub-with-success class (PR keeps job named `gate` with `if: always()`, replaces body with `exit 0`) remains residual — see ADR-0001 *What `gate-verified` catches and does not catch* and issue #53. Do not move the logic back into `tofu-plan.yml`, do not weaken the `select(.name == "gate") | .conclusion == "success"` assertion, and do not rename the in-workflow `gate` job (the assertion matches on that exact name). End-to-end bypass-detection verified 2026-05-15 via canary PR #57 (deleted `tofu-plan.yml`; both required checks went missing; `mergeStateStatus: BLOCKED`).
