...
- **Automerge is only as good as the repo's required checks, and coverage is uneven.** GitHub auto-merge waits on required checks; where there are none it merges immediately, so `platformAutomerge` inherits whatever gate the repo happens to have. Measured 2026-08-21:
...
- **`.github` has no required status checks and now has the auto-merge capability.** It is also the repo holding the preset that every other repo extends, so a change there changes automerge policy fleet-wide. Two things keep this from being a new hole rather than a restatement of today's posture: auto-merge is opt-in per pull request, so nothing enqueues itself, and `.github` carries no manifests for Renovate to update. Do not onboard `.github` to Renovate, and do not enable auto-merge on a `.github` PR by hand — preset changes are exactly the class that wants a human read. A required check on `.github` would be the durable fix and is not in this change.
