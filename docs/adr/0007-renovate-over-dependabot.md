...
- **Two org repos are outside tofu's management and will not receive the baseline change.** Two private repos are absent from `repos_existing.tf`, so they keep `allow_auto_merge = false` and automerge cannot work there even with a required check. `CLAUDE.md` claims that file covers every org repo except the management repo; it does not. Adopting them by `import` is the fix and is not in this change.
