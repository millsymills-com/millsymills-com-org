...
The oldest of those configs dates to 2025-08; ten of the twelve were added between 2026-04 and 2026-06. Run rate is roughly 120 PRs/month from 12 repos. 37% of PRs raised were closed without merging, which is pure review cost with no landed change. Extending the current arrangement to the 42 uncovered repos would roughly quadruple the volume.
...
- One repo splits majors into their own PR, with an inline note recording why: a grouped merge carried `docker/metadata-action` v5→v6 through without anyone reading the changelog.
...
Not automerged, ever: major updates, and anything arriving through `vulnerabilityAlerts`. Majors are held because of the `docker/metadata-action` incident above. Security updates are held for a subtler reason, and it is the reason the twelve existing configs all carry a 7-day cooldown, stated in one of their configs: "a freshly published version is when a compromised release is most likely to still be live." A security alert is exactly the case where the cooldown must be waived for speed, so waiving the cooldown *and* the human at once would remove both defenses simultaneously. Security fixes therefore skip the cooldown and the weekly window, and wait for a person.
...
- **Automerge is only as good as the repo's required checks, and coverage is uneven.** GitHub auto-merge waits on required checks, so `platformAutomerge` inherits whatever gate the repo happens to have. Note which way this fails: a repo with *no* required check does not merge instantly, it does not automerge at all. GitHub only offers auto-merge on a pull request that cannot already be merged, so on a clean squash-only PR the `enablePullRequestAutoMerge` mutation is rejected. At least one required check is what makes automerge *function*; the checks' quality is what makes it *safe*. Measured 2026-08-21:
  ...
  | `.github` | **none** |
  | the five user-account repos with Dependabot today | **none** — four have no branch protection at all; `millsymills.com` has a ruleset but no required checks, and its own `dependabot.yml` header records its CI as `workflow_dispatch`-only |
...
- **The `dependencies` label must exist per repo, and tofu can only reach a fraction of them.** Six of the eight managed repos already carry it; two do not. Renovate warns and continues where it is missing, so this degrades rather than breaks. Not fixed here on purpose: adding the label to `modules/repo-baseline` would cover eight repos out of roughly fifty-four, because the user-account repos are outside the org and outside tofu entirely. A partial fix would leave the same warning on the other forty-six while adding a resource and six `import` blocks. Do it once, for the whole onboarded set, when that set is known.
...
- **No repo is onboarded by this change, and that ordering is forced.** A repo's `renovate.json` extends `local>millsymills-com/.github:renovate-config`, which Renovate cannot resolve — and `renovate-config-validator` cannot check — until the preset is on `.github`'s default branch. Onboarding therefore follows the preset PR rather than accompanying it. Start with the four repos whose required checks are strong enough to gate a merge, per the table above.
- The one config using Dependabot's `pre-commit` ecosystem has a Renovate equivalent, but the `pre-commit` manager ships disabled and neither `config:recommended` nor `config:best-practices` turns it on — the preset extends `:enablePreCommit` explicitly, or the rule matches nothing and no hook is ever updated. Its `pip` ecosystem maps to `pip_requirements`; that repo uses `requirements.txt`, not `uv.lock`, and is the noisiest of the twelve at 168 PRs.
- **Two org repos are outside tofu's management and will not receive the baseline change — and adopting them is blocked, not merely deferred.** Two private repos are absent from `repos_existing.tf`, so they keep `allow_auto_merge = false` and automerge cannot work there even with a required check. `CLAUDE.md` claims that file covers every org repo except the management repo; it does not.

  The obvious fix does not work. `repos_existing.tf` lives in a public repo and has only ever declared public repos; a `repo-baseline` declaration needs the repo's name and description, so adopting these two would publish a private repo's name and its description permanently, in the same change that removed four private repo names for exactly that reason. Org-as-code in a public repo can only manage repos whose existence is already public.

  That leaves three options, and choosing between them is the unfiled ADR-0004 decision on public-only posture (issue #46): keep the two repos unmanaged and correct the `CLAUDE.md` claim; accept publishing their names; or stand up the private companion repo that the drafted posture rejects. This ADR does not decide it. Until it is decided, the accurate statement is that tofu manages the org's public repos, not the org.
