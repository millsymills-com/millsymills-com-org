...
- **Automerge is unsafe on five repos until they have a gate.** `resurgent`, `a2a-security-research`, `ellingson-a2a-signed-card`, and `ubiquiti-research` have no branch protection and no required status checks; `millsymills.com` has a ruleset but no required checks, and its own `dependabot.yml` header records that its CI is `workflow_dispatch`-only. With no required check, GitHub auto-merge merges immediately. Roll automerge out to the six org repos first — they already require `Lint & Format`, `Type Check`, `Bandit Security Scan`, `Test (Python 3.13)`, `Dependency Review`, and `CI Pass` at `required_approving_review_count: 0`, which is the shape automerge needs — and leave `automerge: false` on the user-account repos until each has at least one required check.
